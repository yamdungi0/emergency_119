...
st.set_page_config(
    page_title="119 약물안전 코파일럿 | 상황판",
    page_icon="🚑",
    layout="wide",
    initial_sidebar_state="expanded",
)
theme.inject_css()
theme.render_sidebar("상황판")
...
st.markdown(
    ...
    '<div class="topbar-title">전국 약물안전 상황판</div>'
    '<div class="topbar-sub">상황실 · 3시간 단위 수요예측 및 이상징후</div>'
    "</div>"
    f'<div class="topbar-time">{dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")} KST</div>'
    "</div>",
    unsafe_allow_html=True,
)
